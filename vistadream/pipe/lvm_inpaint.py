...
class Inpaint_Tool:
    ...
    def _llava_prompt(self, frame):
        prompt = "<image>\n \
                USER: In detail, imagine and describe the scene this image is taken from? \
    ...
    def __call__(self, frame: Frame, outpaint_selections=[], outpaint_extend_times=0.0):
        ...
        fooocus_result = self.fooocus(
            ...
            origin_image=image,
            mask_image=mask,
            seed=self.cfg.scene.outpaint.seed,
        )[0]
        torch.cuda.empty_cache()

        # reset the frame for outpainting
        if len(outpaint_selections) > 0.0:
            ...
            inpaint[begin_H : (begin_H + small_H), begin_W : (begin_W + small_W)] *= 0.0
