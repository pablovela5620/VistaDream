...
def main(
    ...
    def get_res(editor_val):
        # editor_val keys: 'background' (PIL.Image), 'layers' (list of PIL), 'composite'
        image = editor_val["background"]
        mask = editor_val["layers"][0]

        # Apply more aggressive resizing to prevent memory issues
        # Limit to 512x512 max resolution while preserving aspect ratio
        max_dimension = 512
    ...
    with gr.Blocks() as demo, gr.Column():
        # image + editable mask in one component
        input_editor = gr.ImageMask(
            type="pil",  # returns dict -> {'background', 'layers', 'composite'}
            sources=["upload", "clipboard"],
            brush=gr.Brush(colors=["#FFFFFF"]),  # optional
            label="Input",
        )
        ...
        run_btn.click(fn=get_res, inputs=input_editor, outputs=[flux_res])
